/**
 * Generated by Kubb (https://kubb.dev/).
 * Do not edit manually.
 */

import { z } from "@/lib/zod/index.ts";

import { badRequestResponseSchemaSchema } from "./bad-request-response-schema-schema.ts";
import { createIdeasListExportBodySchemaSchema } from "./create-ideas-list-export-body-schema-schema.ts";
import { createIdeasListExportResponseSchemaSchema } from "./create-ideas-list-export-response-schema-schema.ts";
import { forbiddenResponseSchemaSchema } from "./forbidden-response-schema-schema.ts";
import { internalServerErrorResponseSchemaSchema } from "./internal-server-error-response-schema-schema.ts";
import { notFoundResponseSchemaSchema } from "./not-found-response-schema-schema.ts";
import { unauthorizedResponseSchemaSchema } from "./unauthorized-response-schema-schema.ts";

export const postApiV1IdeasListsByIdeasListIdExportsPathParamsSchema = z.object(
  {
    ideasListId: z.uuid(),
  },
);

/**
 * @description Ideas list export created successfully
 */
export const postApiV1IdeasListsByIdeasListIdExports201Schema = z
  .lazy(() => createIdeasListExportResponseSchemaSchema)
  .describe("Create ideas list export response description");

/**
 * @description Bad request
 */
export const postApiV1IdeasListsByIdeasListIdExports400Schema = z
  .lazy(() => badRequestResponseSchemaSchema)
  .describe("Bad request response description");

/**
 * @description Unauthorized
 */
export const postApiV1IdeasListsByIdeasListIdExports401Schema = z
  .lazy(() => unauthorizedResponseSchemaSchema)
  .describe("Unauthorized response description");

/**
 * @description Forbidden
 */
export const postApiV1IdeasListsByIdeasListIdExports403Schema = z
  .lazy(() => forbiddenResponseSchemaSchema)
  .describe("Forbidden response description");

/**
 * @description Not found
 */
export const postApiV1IdeasListsByIdeasListIdExports404Schema = z
  .lazy(() => notFoundResponseSchemaSchema)
  .describe("Not found response description");

/**
 * @description Internal server error
 */
export const postApiV1IdeasListsByIdeasListIdExports500Schema = z
  .lazy(() => internalServerErrorResponseSchemaSchema)
  .describe("Internal server error response description");

export const postApiV1IdeasListsByIdeasListIdExportsMutationRequestSchema = z
  .lazy(() => createIdeasListExportBodySchemaSchema)
  .describe("Create ideas list export body description");

export const postApiV1IdeasListsByIdeasListIdExportsMutationResponseSchema =
  z.lazy(() => postApiV1IdeasListsByIdeasListIdExports201Schema);
