/**
 * Generated by Kubb (https://kubb.dev/).
 * Do not edit manually.
 */

import { z } from "@/lib/zod/index.ts";

import { badRequestResponseSchemaSchema } from "./bad-request-response-schema-schema.ts";
import { forbiddenResponseSchemaSchema } from "./forbidden-response-schema-schema.ts";
import { generateScenarioMetadataResponseSchemaSchema } from "./generate-scenario-metadata-response-schema-schema.ts";
import { internalServerErrorResponseSchemaSchema } from "./internal-server-error-response-schema-schema.ts";
import { notFoundResponseSchemaSchema } from "./not-found-response-schema-schema.ts";
import { paymentRequiredResponseSchemaSchema } from "./payment-required-response-schema-schema.ts";
import { unauthorizedResponseSchemaSchema } from "./unauthorized-response-schema-schema.ts";

export const postApiV1ScenariosByScenarioIdMetadataGeneratePathParamsSchema =
  z.object({
    scenarioId: z.uuid(),
  });

/**
 * @description Scenario metadata generation enqueued
 */
export const postApiV1ScenariosByScenarioIdMetadataGenerate202Schema = z
  .lazy(() => generateScenarioMetadataResponseSchemaSchema)
  .describe("Generate scenario metadata response description");

/**
 * @description Bad request
 */
export const postApiV1ScenariosByScenarioIdMetadataGenerate400Schema = z
  .lazy(() => badRequestResponseSchemaSchema)
  .describe("Bad request response description");

/**
 * @description Unauthorized
 */
export const postApiV1ScenariosByScenarioIdMetadataGenerate401Schema = z
  .lazy(() => unauthorizedResponseSchemaSchema)
  .describe("Unauthorized response description");

/**
 * @description Payment required
 */
export const postApiV1ScenariosByScenarioIdMetadataGenerate402Schema = z
  .lazy(() => paymentRequiredResponseSchemaSchema)
  .describe("Payment required response description");

/**
 * @description Forbidden
 */
export const postApiV1ScenariosByScenarioIdMetadataGenerate403Schema = z
  .lazy(() => forbiddenResponseSchemaSchema)
  .describe("Forbidden response description");

/**
 * @description Not found
 */
export const postApiV1ScenariosByScenarioIdMetadataGenerate404Schema = z
  .lazy(() => notFoundResponseSchemaSchema)
  .describe("Not found response description");

/**
 * @description Internal server error
 */
export const postApiV1ScenariosByScenarioIdMetadataGenerate500Schema = z
  .lazy(() => internalServerErrorResponseSchemaSchema)
  .describe("Internal server error response description");

export const postApiV1ScenariosByScenarioIdMetadataGenerateMutationResponseSchema =
  z.lazy(() => postApiV1ScenariosByScenarioIdMetadataGenerate202Schema);
