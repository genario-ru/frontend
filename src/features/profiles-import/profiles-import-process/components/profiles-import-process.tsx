...
export function ProfilesImportProcess({
  ...
  return (
    <aside
      className={cn(
        "from-neutral-8 to-accent-6 dark:from-neutral-1 dark:to-accent-6 text-neutral-1 dark:text-neutral-8 rounded-5 flex w-full flex-col gap-6 bg-linear-to-br p-5 lg:w-[480px] lg:shrink-0",
        className,
      )}
    >
      ...
      <ol className="flex flex-col gap-6">
        {profilesImportProcessSteps.map((step) => (
          <li key={step.number} className="flex items-start gap-4">
            <div className="rounded-3 flex shrink-0 items-center justify-center bg-white/30 p-3">
              <span className="text-xl font-semibold">{step.number}</span>
            </div>
            <div className="flex flex-col gap-1">
              <p className="font-medium">{step.title}</p>
              <p className="text-neutral-1/75 dark:text-neutral-8/75 text-sm">
                {step.description}
              </p>
            </div>
          </li>
