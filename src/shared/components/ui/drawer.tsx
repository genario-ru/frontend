...
type DrawerContentProps = PropsWithClassName<PropsWithChildren>;
...
export function DrawerSection({
  ...
  if (title) {
    return (
      <div className="rounded-4 bg-neutral-3 flex flex-col">
        <p className="px-4 py-1">{title}</p>
        {content}
      </div>
    );
  }
  ...
  return (
    <DrawerBase.Portal>
      ...
      <DrawerBase.Viewport className="fixed inset-0 flex items-end justify-center">
        <DrawerBase.Popup className="bg-neutral-1 rounded-t-5 -mb-12 max-h-[90dvh] w-full transform-[translateY(var(--drawer-swipe-movement-y))] touch-auto overflow-y-auto overscroll-contain pb-12 transition-transform duration-300 ease-[cubic-bezier(0.32,0.72,0,1)] data-ending-style:transform-[translateY(calc(100%-3rem+2px))] data-ending-style:duration-[calc(var(--drawer-swipe-strength)*300ms)] data-starting-style:transform-[translateY(calc(100%-3rem+2px))] data-swiping:select-none">
          <DrawerBase.Content
            className={cn("bg-neutral-2 flex w-full flex-col gap-2", className)}
          >
            {children}
          </DrawerBase.Content>
        </DrawerBase.Popup>
